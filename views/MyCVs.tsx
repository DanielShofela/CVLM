...
import { FileText, Download, Wand2, Share, Trash2, Plus, Heart, ArrowRight, Clock, CheckCircle, Loader2, Archive } from 'lucide-react';
...
interface MyCVsProps {
  ...
  onUpdateRequest: (id: string, status: 'pending' | 'processing' | 'completed') => void;
  onDeleteRequest: (id: string) => void;
  onArchiveRequest: (id: string) => void;
  onUnarchiveRequest: (id: string) => void;
}

export const MyCVs: React.FC<MyCVsProps> = ({ cvs, requests, onDeleteCV, templates, onToggleFavorite, onCreateCV, onUpdateRequest, onDeleteRequest, onArchiveRequest, onUnarchiveRequest }) => {
                ...
                <div className="flex items-center gap-2">
                  ...
                  {req.status === 'pending' && (
                    <button
                      ...
                      className="text-xs bg-emerald-100 text-emerald-700 px-3 py-1 rounded-md font-semibold hover:bg-emerald-200"
                      title="Marquer reçu"
                    >
                      Marquer reçu
                    </button>
                    ...
                    <div className="flex gap-1">
                      {req.status !== 'archived' && (
                        <button
                          ...
                          className="text-xs bg-slate-100 text-slate-600 p-1.5 rounded-md hover:bg-slate-200 transition-colors"
                          title="Archiver"
                        >
                          <Archive size={16} />
                        </button>
                      ...
                      {req.status === 'archived' && (
                        <button
                          onClick={() => onUnarchiveRequest(req.id)}
                          className="text-xs bg-blue-100 text-blue-600 p-1.5 rounded-md hover:bg-blue-200 transition-colors"
                          title="Désarchiver"
                        >
                          <Archive size={16} />
                        </button>
                      ...
                      <button
                        ...
                        className="text-xs bg-red-100 text-red-600 p-1.5 rounded-md hover:bg-red-200 transition-colors"
                        title="Supprimer"
                      >
                        <Trash2 size={16} />
                      </button>
        ...
        <div className="fixed inset-0 z-50 flex items-center justify-center p-4">
          ...
          <div className="bg-white rounded-2xl w-full max-w-sm relative z-10 shadow-2xl p-6 animate-fade-in-up">
            <div className="flex justify-between items-center mb-4">
              <h2 className="text-lg font-bold text-red-600">Supprimer la demande</h2>
              <button onClick={() => setDeleteConfirm(null)} className="p-1 rounded-full hover:bg-slate-100">
